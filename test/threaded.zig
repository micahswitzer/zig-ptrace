...
fn signal_handler(sig: c_int, info: *const std.posix.siginfo_t, context: ?*const anyopaque) callconv(.C) void {
    _ = sig;
    _ = info;
    _ = context;
...
threadlocal var pid: std.posix.pid_t = 0;

fn thread_main(index: usize) !void {
    const writer = std.io.getStdOut().writer();
    pid = std.os.linux.gettid();
    std.posix.nanosleep(index, 0);
    while (true) {
